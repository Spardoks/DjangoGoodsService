...
...  
Also ToDo: email notif atter order created, order accepted  
Also ToDo: пока договариваемся для упрощения работы в один заказ добавлять только заказы одного магазина и оповещать магазином об изменении статуса заказов сразу по всем товарам из его заказа - но стоит обобщить в дальнейшем, так как это более корректный путь работы сервиса